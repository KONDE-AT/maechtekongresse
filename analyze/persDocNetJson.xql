...
let $result := 
    <result>{
        ...
        for $doc at $pos in collection($app:editions)//tei:TEI
            let $title := $doc//tei:titleStmt/tei:title[@type='main'][1]/text()
            return
                <nodes>
                    <id>{$pos}</id>
                    <title>{$title}</title>
    ...
    }
    {
        ...
        for $doc at $pos in collection($app:editions)//tei:TEI
            let $title := $doc//tei:titleStmt/tei:title[@type='main'][1]
            let $docID := $pos
            for $person in $doc//tei:particDesc/tei:listPerson/tei:person/tei:persName
